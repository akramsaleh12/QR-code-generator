...
# ----------------------------------------------------------------------------
with st.sidebar:
    st.image("assets/ak11.png", width=150)
    st.markdown('<div class="profile-name">Akram Elsayed</div>', unsafe_allow_html=True)
    st.markdown('<div class="profile-role">App Developer</div>', unsafe_allow_html=True)
    st.divider()
...
if st.button("Generate QR Code", type="primary"):
    if not url.strip():
        ...
        qr = segno.make(clean_url, error="m")

        # Render to PNG bytes for display + download (scale = pixel size per module)
        buf = BytesIO()
